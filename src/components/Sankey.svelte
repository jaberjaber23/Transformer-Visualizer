<script lang="ts">
	...
	const backPathMap: PathMap = {
		attention: [
			...
			},
			{
				...
				gradientId: 'gray-gray',
				// fill: theme.colors.gra[defaultGradientBrightness],
				opacity: ATTENTION_HEAD_BACK
			},
			{
	...
	const animationTest = () => {
		const grad = document.querySelector('#gray-gray');
		const stop = grad?.querySelectorAll('stop')[1];
		gsap.fromTo(
			stop,
			{ attr: { offset: '0%', ['stop-color']: theme.colors.gray[100] } },
			{
				attr: { offset: '100%', ['stop-color']: theme.colors.gray[200] },
				duration: 2,
				repeat: 100
			}
		);
