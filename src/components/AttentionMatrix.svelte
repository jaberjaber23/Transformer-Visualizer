<script lang="ts">
	...
	const expandAttention = () => {
		...
		// const grad = document.querySelector('#gray');
	...
	$: qkColorScale = (d, i) => {
		return d3
			...
			.range(['white', theme.colors['black'][700]])(d);
	};
	const maskedColorScale = (d, i) => {
		return d3.scaleLinear().domain([-3, 3]).range(['white', theme.colors['black'][700]])(d);
	};
	const softmaxColorScale = (d, i) => {
		return d3.interpolate('white', theme.colors['black'][700])(d);
	};
</script>
...
<style lang="scss">
	...
	.color-scale {
		...
		.bar {
			...
			background: linear-gradient(90deg, white 0%, theme('colors.gray.700') 100%);
		}
		.val {
			flex-shrink: 0;
