				...
				<div class="formula-container mt-4 w-[400] flex-col justify-around gap-1 bg-gray-50 p-2">
					<div class="even-pos flex h-10 flex-1 items-center justify-between gap-4 p-1">
						<div class="flex items-center justify-center gap-1">
							...
							{#if $hoveredCol % 2 === 0}
								<span class="flex w-8 justify-center rounded-sm border border-black-400">sin</span>
							{:else}
								<span class="flex w-8 justify-center rounded-sm border border-black-400">cos</span>
							{/if}
							<span>(</span>
							<span class="flex w-8 justify-center rounded-sm border border-black-400"
								>{$hoveredCol ? $hoveredCol : 'pos'}</span
							>
							<span>/ 10000 </span>
							<sup
								>(2 *
								<span class="mr-1 rounded-sm border border-black-400 pl-2 pr-2"
							...
							<span class="w-8 flex justify-center border border-black-400 rounded-sm">{$hoveredRow ? $hoveredRow : 'i'}</span>
							<span>/ 768 ) )</span> -->
							<!-- <span>=</span> -->
						</div>
						<!-- <div>=</div> -->
...
<style lang="scss">
	...
	.highlight-token-box {
		...
		transition: 0.1s;
		// box-shadow: 0 0 0 1px black, 0 0 0 3px black;
	}
